    ...
    match beamings {
        VoicesBeamings::One(voice_beamability) => match voice_beamability {
            VoiceBeamability::Unbeamable => {
                println!("upper voice not beamable");
            }
            VoiceBeamability::Beamable(items) => {
                println!("single voice beamable");
                for item in items {
                    println!("- item.internal_direction:{:?}", item.internal_direction);
                }
            }
        },
        VoicesBeamings::Two(upper_beamability, lower_beamability) => {
            match upper_beamability {
                // upper voice not beamable, for example barpause
                VoiceBeamability::Unbeamable => {
                    println!("upper voice not beamable");
                }
                // upper voice beamable
                VoiceBeamability::Beamable(upper_items) => {
                    println!("upper voice beamable");
            ...
            match lower_beamability {
                // lower voice not beamable, for example barpause
                VoiceBeamability::Unbeamable => {
                    println!("lower voice not beamable");
                }
                // lower voice beamable
                VoiceBeamability::Beamable(lower_items) => {
                    println!("lower voice beamable");
...
// pub fn complexes_from_voices2(voices: &Voices) -> Result<Vec<Complex>> {
pub fn complexes_from_voices2<'a>(
    voices: &'a Voices,
...
/*
pub fn complexes_from_voices(voices: &Vec<Voice>) -> Result<Vec<Complex>> {
    let mut complexes: Vec<Complex> = vec![];
